...
def main():
	...
	APP_CNTR =  Controller(cin, cout).add("sorting", SORT_CNTR.silent_command(SORT_CNTR.start)).add("fibonacci", FIB_CNTR.silent_command(FIB_CNTR.start)).add("search", SRCH_CNTR.silent_command(SRCH_CNTR.start)).add("unique holder", UNQ_CNTR.silent_command(UNQ_CNTR.start))
	APP_CNTR.start()
